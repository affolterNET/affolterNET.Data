...
namespace ExampleVersionUserDate.Data
{
    ...
    public class ExampleVersionUserDate_T_DemoTable : IDtoBase
    {
        ...
        public string GetSelectCommand(int maxCount = 1000, params string[] excludedColumns)
        {
            ...
            return $"select top({maxCount}) {cols.JoinCols()} from ExampleVersionUserDate.T_DemoTable where (@Id is null or [Id]=@Id)";
        }
        ...
        public string GetInsertCommand(bool returnScopeIdentity = false, params string[] excludedColumns)
        {
            ...
            var sql = $"insert into ExampleVersionUserDate.T_DemoTable ({cols.JoinCols()}) values ({cols.JoinCols(true)})";
        ...
        public string GetUpdateCommand(params string[] excludedColumns)
        {
            ...
            return $"update ExampleVersionUserDate.T_DemoTable set {cols.JoinForUpdate()} where [Id]=@Id and [VersionTimestamp]=@VersionTimestamp";
        }
    ...
    public class ExampleVersionUserDate_T_DemoTableType : IDtoBase
    {
        ...
        public string GetSelectCommand(int maxCount = 1000, params string[] excludedColumns)
        {
            ...
            return $"select top({maxCount}) {cols.JoinCols()} from ExampleVersionUserDate.T_DemoTableType where (@Id is null or [Id]=@Id)";
        }

        public string GetInsertCommand(bool returnScopeIdentity = false, params string[] excludedColumns)
        {
            var cols = "[Id], [Name]".GetColumns(excludedColumns);
            var sql = $"insert into ExampleVersionUserDate.T_DemoTableType ({cols.JoinCols()}) values ({cols.JoinCols(true)})";
            if (returnScopeIdentity)
            {
                sql += "; select scope_identity() as id;";
            }
        ...
        {
            var cols = "[Id], [Name]".GetColumns(excludedColumns);
            return $"update ExampleVersionUserDate.T_DemoTableType set {cols.JoinForUpdate()} where [Id]=@Id";
        }
    ...
    public class ExampleVersionUserDate_V_Demo : IViewBase
    {
        ...
        {
            var cols = "[Id], [Message], [Type], [Status], [InsertDate], [InsertUser], [UpdateDate], [UpdateUser], [VersionTimestamp]".GetColumns(excludedColumns);
            return $"select top({maxCount}) {cols.JoinCols()} from ExampleVersionUserDate.V_Demo";
        }
