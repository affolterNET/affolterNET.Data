...
namespace ExampleHistory.Data
{
    ...
    public class ExampleHistory_T_DemoTableType : IDtoBase
    {
        ...
        public string GetSelectCommand(int maxCount = 1000, params string[] excludedColumns)
        {
            ...
            return $"select top({maxCount}) {cols.JoinCols()} from ExampleHistory.T_DemoTableType where (@Id is null or [Id]=@Id)";
        }

        public string GetInsertCommand(bool returnScopeIdentity = false, params string[] excludedColumns)
        {
            var cols = "[Id], [Name]".GetColumns(excludedColumns);
            var sql = $"insert into ExampleHistory.T_DemoTableType ({cols.JoinCols()}) values ({cols.JoinCols(true)})";
            if (returnScopeIdentity)
            {
                sql += "; select scope_identity() as id;";
            }
        ...
        {
            var cols = "[Id], [Name]".GetColumns(excludedColumns);
            return $"update ExampleHistory.T_DemoTableType set {cols.JoinForUpdate()} where [Id]=@Id";
        }
